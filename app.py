...
def login():
    if request.method == 'POST':
        ...
        if not username_input or not password_input:
            return render_template('login.html', error='Brukernavn og passord er påkrevd.')
...
def vote_page(subject_id):
    ...
    cursor.execute("SELECT id, label FROM subject_options WHERE subject_id = %s ORDER BY id", (subject_id,))
    options = cursor.fetchall()
    cursor.execute(
    ...
    return render_template('avstemning.html',
                           subject=subject,
                           options=options,
